...
- `* Title` is the song's title, `** Author` is the subtitle. The marker sits at
  ...
  one counts.
- A blank line starts a new block (verse, chorus, bridge...).
...
- Several chords in one bracket sit in the same spot: `[Em G A]`. A line with
  chords and no words is a chord-only line, and a block made only of those prints
  larger — the usual intro/solo/bridge look.
- Bracket text that is not a real chord — `[Solo]`, `[2x]`, `[N.C.]`, `[||: :||]`
  — is printed above the line exactly as written. Keep it, don't "fix" it.
...
- Chord placement matters most. On the sheet the chords sit on a line above the
  words — put each `[chord]` right before the character it stands over.
...
plainly shows — usually that is just `aspectRatio` and `columns` — and leave the
rest out, so the app's own defaults apply.

| Setting         | Values                                       | Default     | Set it when                                                          |
| --------------- | -------------------------------------------- | ----------- | -------------------------------------------------------------------- |
| `aspectRatio`   | `"A4"`, a ratio like `"4/3"`, or a number    | `"A4"`      | portrait page: `"A4"`. A wide sheet: its width divided by its height |
...
| `padding`       | a number, in em                              | `0.5`       | the margin around the words is unusually wide or tight                |
...
2. **Content** — the markup, in one code block, ready to copy.
3. **Import file** — the JSON below, filled in. Offer it as a `.json` file to
...
- `content` is the same markup as #2, as a single JSON string: `\n` for line
  breaks, `\"` for quotes.
- `id` is any random UUID. The three time fields are the current time in
  milliseconds since 1970 (a 13-digit number).
- `name` is my label for the song in the library — use the title.
...
You get three things back - a settings table, the song text, and the import file.
The import file is the one you need, it has the other two in it.
...
The other way round, without a file: create a new song, paste the song text into
the [editor](/songs/editing.mdx), and set the settings from the table by hand.
