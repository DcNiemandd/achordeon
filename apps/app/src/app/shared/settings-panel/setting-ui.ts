...
/**
 * The one option in a font picker that is not a font.
 *
 * A sentinel rather than a button beside the control: the list is where a user
 * goes when the font they want is not there, so that is where the way to get it
 * belongs. Never stored — the panel intercepts it and re-shows the value the row
 * already had.
 *
 * `@` because a font id is a slug or `custom:<slug>`, so no real one can begin
 * with it. This was briefly a control character, on the same reasoning about what
 * a slug cannot hold — but a NUL is not text, and the file stopped being a text
 * file to git.
 */
export const ADD_FONT = '@add-font';
